            ...
            <div class="row justify-content-md-center">
                <nav class="col-md-auto">
                    <ul class="pagination">
                        <li class="page-item">
                            <a class="page-link"
                               href="/site/searchAlbumsPerformer?page=<?= ($data['pageNum'] - 1 > 0) ? $data['pageNum'] - 1 : '1'; ?>&idPerformer=<?= $data['performer']->getId()?>"
                               aria-label="Previous">
                        ...
                        <?php for ($i = 1; $i <= $data['lastPage']; $i++) { ?>
                            <li class="page-item <?= ($i == $data['pageNum']) ? 'active' : '' ?>"><a class="page-link"
                                                                                                     href="/site/searchAlbumsPerformer?page=<?= $i; ?>&idPerformer=<?= $data['performer']->getId()?>"><?= $i; ?></a>
                            </li>
                        <?php } ?>
                        <li class="page-item">
                            <a class="page-link"
                               href="/site/searchAlbumsPerformer?page=<?= ($data['pageNum'] + 1 <= $data['lastPage']) ? $data['pageNum'] + 1 : $data['lastPage']; ?>&idPerformer=<?= $data['performer']->getId()?>"
                               aria-label="Next">
